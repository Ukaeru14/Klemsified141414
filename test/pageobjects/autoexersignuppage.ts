...
await this.emailaddress.setValue(randomEmailaddress);
await browser.pause(2000);
await this.signupbtn.click();
await browser.pause(2000);
await this.tittlemr.click();
const email= await this.email.getValue();
await expect (email).toEqual(randomEmailaddress);
await expect(this.username).toHaveValue('Clement Ukaeru');
await this.password.setValue('Mummys1991@')
await this.dobday.selectByIndex(18);
await this.dobmonth.selectByVisibleText('June');
await this.dobyear.selectByVisibleText('2000');
